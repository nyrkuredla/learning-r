...
- **Performance metrics and fit evaluation methods** used to evaluate regression models. - how good is the fit?
- **Determine if a model is overfit** and how to prune features to reduce overfit. - overfit = ML model won't generalize well with new cases/new data in production.
- **Transform features** to make them suitable for linear models. "data munging"
...
set.seed(1212)
df_error <- sim.rnorm (n = 1000, mean = 0, sd = 1)
...
plot.reg <- function(df){
  df %>% ggplot(aes(x, y)) + 
    geom_point(size = 2) +
    geom_smooth (aes(x, score), size = 2, color = 'blue') +
    labs(title="X vs. Y")
}
